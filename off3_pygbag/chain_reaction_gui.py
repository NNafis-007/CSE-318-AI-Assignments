...
# Try to import subprocess, but handle web environment gracefully
try:
    import subprocess
except ImportError:
    subprocess = None  # Not available in web environment
...
class ChainReactionGUI:
    def __init__(self):
        ...
        self.RASPBERRY = (200, 80, 150)  # Custom color for instructions
    ...
    def save_game_state(self):
        ...
        if self.game:
            ...
            try:
                ...
                # In web environment, file operations might fail
    ...
    def draw_main_menu(self, screen):
        ...
        game_instructions = [
            "Game Instructions : ",
            "  -- Critical mass of a cell = orthogonally neighboring cells",
            "  -- Click cell to place orb (only same color or empty cell)",
            "  -- reach critical mass -> occupy neighboring cells",
            "  -- delete all opponent cells to win"
        ]
        ...
        for i, instruction in enumerate(game_instructions):
            text = self.font.render(instruction, True, self.RASPBERRY)
            text_rect = text.get_rect(center=(screen.get_width()//2, y_start + 100 + i * 30))
            screen.blit(text, text_rect)
    ...
    def draw_game(self, screen):
        ...
        if self.game.game_over:
            ...
            game_over_surface = self.small_font.render(game_over_text, True, self.BLACK)
            winner_surface = self.font.render(winner_text, True, self.RED if self.game.winner == Player.RED else self.BLUE)
            
            game_over_rect = game_over_surface.get_rect(center=(screen.get_width()//2, start_y + board_height + 70))
            winner_rect = winner_surface.get_rect(center=(screen.get_width()//2, start_y + board_height + 50))
            ...
            # Reset Game button - positioned on the left side of the grid
            reset_text = "Reset Game"
            reset_surface = self.font.render(reset_text, True, self.WHITE)
            reset_bg_rect = pygame.Rect(0, 0, 150, 50)
            
            # Position button on the left side of the grid, vertically centered
            button_x = start_x + 110 # 180 pixels to the left of the grid
            button_y = start_y + (board_height // 2) - 25  # Vertically centered on the grid
            
            # Ensure button doesn't go off-screen
            if button_x < 10:
                button_x = 10
            
            reset_bg_rect.topleft = (button_x, button_y)
            reset_text_rect = reset_surface.get_rect(center=reset_bg_rect.center)
            
            # Draw button background
            pygame.draw.rect(screen, self.DARK_GREEN, reset_bg_rect)
            pygame.draw.rect(screen, self.BLACK, reset_bg_rect, 2)
            screen.blit(reset_surface, reset_text_rect)
            
            # Store button rect for click detection
            self.reset_button_rect = reset_bg_rect
    ...
    def handle_game_click(self, pos: Tuple[int, int], screen_width: int):
        ...
        # Check if game is over and clicking reset button
        if self.game.game_over and self.reset_button_rect and self.reset_button_rect.collidepoint(pos):
    ...
    async def run(self):
        ...
        while running:
            ...
            pygame.display.flip()
            await asyncio.sleep(1/60)  # More precise timing for web
